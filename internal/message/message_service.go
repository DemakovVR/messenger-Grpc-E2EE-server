...
func NewMessageService(repo *MessageRepository, manager *ConnectionManager, auditService *audit.AuditService) *MessageService {
...
func (s *MessageService) DeleteMessage(ctx context.Context, messageID uuid.UUID, userID uuid.UUID) (uuid.UUID, error) {
	chatID, err := s.repo.DeleteMessage(ctx, messageID, userID)
	if err != nil {
		return uuid.Nil, err
	}

	details := "message_id: " + messageID.String()
	action := "/message.MessageService/DeleteMessage"
	_ = s.auditService.CreateLog(ctx, userID, &chatID, action, &details)
...
func (s *MessageService) EditMessage(ctx context.Context, messageID uuid.UUID, userID uuid.UUID, content string, isEncrypted bool) (uuid.UUID, error) {
	chatID, err := s.repo.EditMessage(ctx, messageID, userID, content, isEncrypted)
	if err != nil {
		return uuid.Nil, err
	}

	details := "message_id: " + messageID.String()
	action := "/message.MessageService/EditMessage"
	_ = s.auditService.CreateLog(ctx, userID, &chatID, action, &details)
